...
export default function StudentTable({ students, onEdit, onDelete, onViewProfile ,  refreshStudents}: Props) {
  const theme = useTheme();
  const [inProgress, setInProgress] = useState(false);
  const apiRef = useGridApiRef();
  const lastSyncedRef = useRef<string | null>(null);

  const [studentRows, setStudentRows] = useState<Student[]>(students); // Add this
useEffect(() => {
  setStudentRows(students); // Sync from props
}, [students]);
  ...
  const toggleReminder = async (id: string) => {
    try {
      ...
      setStudentRows((prev) =>
        prev.map((s) =>
          s._id === id ? { ...s, autoReminder: !s.autoReminder } : s
        )
      );
      refreshStudents?.(); // optional, for future sync
    } catch (err) {
  ...
  const rows = studentRows.map((s) => ({ id: s._id, ...s }));
    ...
    <>
      ...
      >
        <DataGrid
          ...
          showToolbar
          onExcelExportStateChange={(newState : any) =>
